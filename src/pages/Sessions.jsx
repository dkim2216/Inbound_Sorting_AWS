...
export default function Sessions({ sessions, onSessionSelected, onRefresh, loading, user, isAdmin }) {
  ...
  const confirmAndClose = async () => {
    ...
    try {
      ...
      if (res.ok) { showMsg('success', 'Session closed. Final report emailed.'); onRefresh(); }
      else { const d = await res.json(); showMsg('error', d.error || 'Failed to close session.'); }
    } catch (err) { showMsg('error', err.message); }
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      {confirmClose && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background: 'rgba(13,27,75,0.55)' }} onClick={() => setConfirmClose(null)}>
          <div className="bg-white rounded-2xl p-6 shadow-xl max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-500 mb-6">
              This marks the session as <strong>closed</strong> and sends a final report email. It can be reopened by an admin if needed.
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={confirmAndClose} className="flex-1 py-2.5 rounded-xl text-white font-semibold text-sm" style={{ background: CS_NAVY }}>Close Session</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          style={{ background: 'rgba(13,27,75,0.55)' }} onClick={() => setConfirmDelete(null)}>
          <div className="bg-white rounded-2xl p-6 shadow-xl max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-2" style={{ color: '#DC2626' }}>Delete this session?</h3>
            <p className="text-sm text-gray-500 mb-6">
              This <strong>permanently deletes</strong> the session and all its manifest data. This cannot be undone.
            </p>
            <div className="flex gap-3">
            ...
            return (
              <button
                ...
                className={`bg-white rounded-2xl border p-4 md:p-6 transition-all text-left shadow-sm ${
                  isClosed ? 'opacity-80 cursor-default' : 'hover:shadow-md'
                }`}
                  ...
                  <div className="flex flex-col gap-2 min-w-[150px]">
                    {isClosed ? (
                      <>
                        ...
                        {isAdmin && (
                          <button
                            onClick={(e) => handleReopen(session.id, e)}
                        ...
                        {isAdmin && (
                          <button
                            ...
                            disabled={isReopening || isDeleting}
                            className="flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-sm font-medium disabled:opacity-50 transition-colors border"
                      ...
                      <>
                        {/* Active pill */}
                        <div className="text-xs font-semibold px-3 py-1 rounded-full text-center"
                        ...
                        {isAdmin && (
                          <button
                            onClick={(e) => handleDelete(session.id, e)}
                            disabled={isSending || isClosing || isDeleting}
                            className="flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-sm font-medium disabled:opacity-50 transition-colors border"
                            style={{ borderColor: '#FECACA', color: '#DC2626', background: '#FFF5F5' }}
                          >
                            <Trash2 size={13} />
